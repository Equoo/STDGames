...
impl IgdbClient {
	...
	pub fn new(client_id: &str, client_secret: &str) -> Result<Self> {
		...
		let token = res.json::<IgdbApiAuthResponse>()?.access_token;
		Ok(Self {
	...
	fn multiquery<T>(&self, fields: &[&str], games: &[u32]) -> Result<Vec<T>>
	where
		T: serde::de::DeserializeOwned + Clone,
	{
		...
		let res = self.reqwest_client.post("https://api.igdb.com/v4/multiquery")
			...
			.send()?.error_for_status()?;
		let data = res.json::<[IgdbApiMultiqueryResponse<T>; 1]>()?;
	...
	pub fn load_igdb_games(&mut self, games: &Vec<Game>) -> Result<()> {
		...
		let fields = [
			...
			"videos.video_id", "websites.url", "websites.type",
			// "cover.width", "cover.height", "videos.name",
			// "artworks.width", "artworks.height",
			// "screenshots.width", "screenshots.height",
		];
		let igdb_games: Vec<IgdbGameInfos> = self.multiquery(&fields, &igdb_ids)?;
		...
		if let Some(igdbid) = meta.igdbid {
			if let Some(game_info) = self.cache.get(&igdbid) {
				...
				meta.cover = data.cover
					.map(|cover| format!("https://images.igdb.com/igdb/image/upload/t_{}/{}.jpg", "cover_big_2x", cover.image_id));

				meta.hero = data.artworks
					.and_then(|mut artworks| if !artworks.is_empty() {Some(artworks.remove(0))} else {None})
					.map(|artwork| artwork.clone())
					.map(|artwork| format!("https://images.igdb.com/igdb/image/upload/t_{}/{}.jpg", "1080p_2x", artwork.image_id));

				meta.screenshots = data.screenshots
					.map(|screenshots| screenshots.into_iter().map(
						|screenshot| format!("https://images.igdb.com/igdb/image/upload/t_{}/{}.jpg", "1080p_2x", screenshot.image_id)
					).collect());

				meta.movies = data.videos
