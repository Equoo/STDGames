...
impl SteamAssetsClient {
    ...
    pub async fn get_icon_hash(&self, app_id: u32, user_id: u64) -> Result<Option<String>, Box<dyn std::error::Error>> {
    ...
    pub async fn get_game_assets(&self, app_id: u32) -> Result<GameAssets, Box<dyn std::error::Error>> {
        ...
        let app_response = response.apps
        ...
        let app_data = app_response.data
            .as_ref()
        ...
        let assets = GameAssets {
            ...
            movies: app_data.movies
                .as_ref()
                .map(|movies| movies.iter().map(|m| MovieAsset {
                    name: m.name.clone(),
                ...
                .unwrap_or_default(),
            // CDN asset URLs
            library_hero: format!("https://cdn.akamai.steamstatic.com/steam/apps/{}/library_hero.jpg", app_id),
            library_600x900: format!("https://cdn.akamai.steamstatic.com/steam/apps/{}/library_600x900.jpg", app_id),
            library_capsule_231x87: format!("https://cdn.akamai.steamstatic.com/steam/apps/{}/capsule_231x87.jpg", app_id),
            library_capsule_616x353: format!("https://cdn.akamai.steamstatic.com/steam/apps/{}/capsule_616x353.jpg", app_id),
            icon: format!("https://cdn.akamai.steamstatic.com/steamcommunity/public/images/apps/{}/icon.jpg", app_id), // Placeholder - use get_icon_with_hash for actual icon
            logo: format!("https://cdn.akamai.steamstatic.com/steam/apps/{}/logo.png", app_id),
        };
    ...
    pub async fn get_icon_with_hash(&self, app_id: u32) -> Result<Option<String>, Box<dyn std::error::Error>> {
		for uid in self.userids {
			if let Some(icon_hash) = self.get_icon_hash(app_id, uid).await? {
				println!("Found icon hash for app {} with user {}: {}", app_id, uid, icon_hash);
    ...
    pub async fn get_game_assets_with_icons(&self, app_id: u32) -> Result<GameAssets, Box<dyn std::error::Error>> {
