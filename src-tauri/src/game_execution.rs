use std::{clone, collections::HashMap, env, error::Error, io, ops::Deref, os::unix::process::CommandExt, path::Path, process::{Child, Command, Stdio}, vec};
...
impl GameExecution {
    ...
    pub fn junest_run(
        ...
        let overlay_str = if let Some(o) = overlay {
            ...
            command.push(format!("/tmp/stdgames/work").to_string());
        ...
        println!("Running command: {} AND {:?}", format!(
				...
				{overlay_str}
			", conf.username, conf.junest_bind, conf.temp_junest_home_dir
            ), command);
        Command::new(conf.junest_bin.clone())
            .envs(environ)
            .arg("-b")
            .arg(format!(
                "--bind /sgoinfre /sgoinfre
            ...
            .args(command)
            .exec();
        Err(io::Error::new(io::ErrorKind::Other, "Failed to execute command"))
            //.spawn()
    }
    ...
    pub fn run(
        ...
        name: String,
        game: &GameLaunchData,
    ) -> Result<&mut GameProcess, Box<dyn Error>> {
		...
		if game.prestart.is_some() {
			...
			Self::junest_run(prestart_command, environ.clone(), &overlay)?.wait()?;
		}

        let process = Self::junest_run(command, environ, &overlay)?;
