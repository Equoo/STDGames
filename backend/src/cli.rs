...
pub fn init_cli(cli: &Cli, library: &Vec<Game>, _game_exec: &mut GameExecution) -> Result<()> {
    ...
    match &cli.command {
        Some(Commands::Run { game }) => {
            ...
            let mut child = GameExecution::build_command(&game, &launch_data)?.spawn()?;
        ...
        Some(Commands::Bash { game }) => {
            ...
            let err = GameExecution::build_command(&game, &launch_data)?.exec();
